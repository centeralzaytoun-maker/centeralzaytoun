...
export default function StudentsPage() {
  ...
  const fetchStudents = async ({ queryKey }) => {
    ...
    const [_key, { page, pageSize, search, grade, course, isExempt, group }] = queryKey;
    ...
    if (group) {
        ...
        if (selectedGroup) {
            params.append('groupCourseId', selectedGroup.course_id);
        }
    }
    ...
    try {
      ...
      const rows = allFilteredStudents.map(student => {
        ...
        return {
          ...
          'تاريخ التسجيل': student.created_at ? new Date(student.created_at).toLocaleString('ar-EG', { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-',
          'المواد والمدرسين': student.enrolled_courses ? student.enrolled_courses
              .filter(cid => filterCourse ? cid === filterCourse : true)
              .map(cid => {
          ...
          'المجموعات': student.enrolled_courses ? student.enrolled_courses
              .filter(cid => filterCourse ? cid === filterCourse : true)
              .map(cid => {
          ...
          'الخصومات المحددة': student.enrolled_courses ? student.enrolled_courses
              .filter(cid => filterCourse ? cid === filterCourse : true)
              .map(cid => {
...
const handlePrintMultipleCards = () => {
    ...
    if (!targetStudents || targetStudents.length === 0) {
        toast.error("لا يوجد طلاب لطباعة كارنيهاتهم");
        return;
    }
...
const handlePrintList = () => {
    ...
    const sortedStudentsForPrint = [...filteredStudents].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
        ...
        <html dir="rtl">
            ...
            <body>
                ...
                <div class="meta">
                    ...
                    <span>👥 عدد الطلاب: ${sortedStudentsForPrint.length}</span>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        ${sortedStudentsForPrint.map((s, index) => `
...
>
      ...
      <div className="print:hidden">
        ...
        <div className="bg-white p-3 md:p-4 rounded-xl shadow-sm border border-gray-200 mb-4 md:mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {/* 🖨️ زر طباعة كارنيهات الدفعة الحالية */}
            <button 
                onClick={handlePrintMultipleCards}
                className="mt-2 md:mt-0 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition shadow-md bg-blue-600 text-white hover:bg-blue-700"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mt-3">
                ...
                <div className="relative">
                    ...
                    <select 
                        value={filterGrade} 
                        onChange={e => { setFilterGrade(e.target.value); setFilterCourse(''); setFilterGroup(''); }}
                ...
                <select 
                    value={filterCourse} onChange={e => { setFilterCourse(e.target.value); setFilterGroup(''); }}
                ...
                <div className="relative">
                    <FaUsers className="absolute right-3 top-3.5 text-gray-400" />
                    <select 
                        value={filterGroup} onChange={e => setFilterGroup(e.target.value)}
                        className="w-full p-2.5 pr-10 min-h-[44px] border-2 border-gray-300 rounded-lg outline-none focus:border-blue-500 bg-white text-sm md:text-base text-gray-900 opacity-100 shadow-sm appearance-none"
                        disabled={!filterCourse}
                    >
                        <option value="" className="text-gray-900">-- كل المجموعات --</option>
                        {groups.filter(g => g.course_id === filterCourse).map(g => (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 md:mb-4 gap-3">
            <h3 className="text-base md:text-xl font-bold text-gray-700 flex items-center gap-2">
    ...
    return (
        ...
        <div key={student.id} className="bg-white p-3 md:p-5 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition flex flex-col gap-3 md:gap-4 relative">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 bg-gray-50/50 p-3 rounded-xl border border-gray-100">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <div className="flex flex-col">
                              ...
                              <span className="font-bold text-gray-800">{student.created_at ? new Date(student.created_at).toLocaleString('ar-EG', { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '---'}</span>
                           </div>
                        </div>
                        <div className="hidden md:block w-px h-8 bg-gray-200"></div>
                        <div className="flex items-center gap-2">
