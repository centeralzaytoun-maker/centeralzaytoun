...
export default async function AdminLayout({ children }) {
  ...
  // 2️⃣ نفس كود جلب البيانات بتاعك بالظبط (بدون تغيير)
  ...
  try {
    ...
    const { data: settings } = await supabase
      ...
      .select('primary_color, center_name, logo_url')
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 md:gap-5 pr-14 md:pr-0">
              <div className="relative group">
                ...
                {logoUrl ? (
                  <img src={logoUrl} alt={centerName} className="relative h-11 w-11 md:h-14 md:w-14 rounded-xl object-contain bg-white/90 p-1.5 shadow-2xl transition-transform hover:scale-105" />
                ) : (
              ...
              <div className="flex flex-col">
                <h1 className="text-lg md:text-2xl font-black text-white tracking-tight drop-shadow-sm">{centerName}</h1>
