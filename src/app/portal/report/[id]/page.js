      ...
      <div className="sticky top-0 z-[100] bg-white/90 backdrop-blur-xl border-b border-gray-100 print:hidden">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{backgroundColor: centerConfig.primary_color}}>
                <FaGraduationCap size={20} />
             </div>
             <span className="font-black text-gray-800 tracking-tight text-sm hidden md:block">نظام المتابعة الأكاديمية - {centerConfig.center_name}</span>
          </div>
      ...
      <div id="printable-area" className="max-w-5xl mx-auto px-4 md:px-0 mt-8">
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl shadow-blue-900/5 border border-gray-100 overflow-hidden print:shadow-none print:border-none print:rounded-none">
          ...
          <div className="relative p-6 md:p-14 border-b-4 md:border-b-8 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8 md:gap-10" style={{borderColor: centerConfig.primary_color}}>
             ...
             <div className="flex items-center gap-6 md:gap-10 flex-col md:flex-row text-center md:text-right relative z-10 w-full md:w-auto">
                ...
                {centerConfig.logo_url ? (
                  <div className="w-24 h-24 md:w-32 md:h-32 p-3 bg-white rounded-3xl md:rounded-[2.5rem] shadow-2xl border border-gray-50 flex items-center justify-center overflow-hidden transition-transform hover:scale-105">
                    <img src={centerConfig.logo_url} className="w-full h-full object-contain" alt="Logo" />
                  </div>
                ) : (
                  <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl md:rounded-[2.5rem] flex items-center justify-center text-4xl md:text-5xl font-black text-white shadow-2xl" style={{backgroundColor: centerConfig.primary_color}}>
                    {centerConfig.center_name?.substring(0,1)}
                  </div>
                )}
                ...
                <div className="space-y-3">
                   <div className="flex flex-col gap-1">
                      <h1 className="text-3xl md:text-5xl font-black text-gray-900 tracking-tighter leading-tight md:leading-none">{centerConfig.center_name}</h1>
                      <div className="flex items-center gap-3 justify-center md:justify-start mt-2">
                        <div className="h-[2px] w-8 md:w-12 rounded-full" style={{backgroundColor: centerConfig.primary_color}}></div>
                        <p className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em]" style={{color: centerConfig.primary_color}}>التميز في الإدارة والتعليم</p>
                      </div>
                   </div>
             ...
             {/* QR ID Section */}
             <div className="flex flex-col items-center gap-3 p-4 md:p-5 bg-white rounded-3xl md:rounded-[2rem] border-2 border-dashed border-gray-100 relative z-10">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[9px] md:text-[10px] font-black mt-1" style={{color: centerConfig.primary_color}}>{student.unique_id}</p>
                </div>
             </div>
          </div>
          ...
          <div className="p-6 md:p-16">
            ...
            <div className="bg-gray-50/50 p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border border-gray-100 flex flex-col lg:flex-row justify-between items-center gap-8 mb-10 md:mb-14">
               <div className="flex items-center gap-4 md:gap-6 w-full lg:w-auto">
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-2xl md:text-3xl font-black shadow-sm" style={{color: centerConfig.primary_color}}>
                  ...
                  <div className="flex-1 text-right">
                    ...
                    <h2 className="text-lg md:text-2xl font-black text-gray-900 leading-none">{student.name}</h2>
            ...
            <div className="section-container">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mb-10 md:mb-16 print:flex print:flex-wrap print:gap-x-4">
                {/* Discipline Score */}
                <div className="relative group bg-white border-2 rounded-2xl md:rounded-[3rem] p-5 md:p-8 text-center shadow-xl shadow-blue-900/5 transition-transform hover:-translate-y-1 print:flex-1 print:min-w-[20%]" style={{borderColor: centerConfig.primary_color}}>
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 md:px-4 py-0.5 md:py-1 rounded-full border-2 text-[8px] md:text-[10px] font-black uppercase tracking-widest print:static print:mb-2" style={{borderColor: centerConfig.primary_color, color: centerConfig.primary_color}}>الالتزام</div>
                    <span className="text-3xl md:text-5xl font-black leading-none" style={{color: centerConfig.primary_color}}>{Math.round((stats.attended / (reportData.length || 1)) * 100)}%</span>
            ...
            <div className="section-container mb-12 md:mb-16">
               <div className="flex items-center gap-4 md:gap-5 mb-8 md:mb-10 border-r-8 pr-4 md:pr-6" style={{borderColor: centerConfig.primary_color}}>
                  <h3 className="text-xl md:text-2xl font-black text-gray-900 leading-none">سجل المتابعة والالتزام المالي</h3>
               </div>
               <div className="bg-white border border-gray-100 rounded-2xl md:rounded-[2.5rem] shadow-sm overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm text-right border-collapse min-w-[600px] md:min-w-full">
                       ...
                       <tbody className="divide-y divide-gray-50">
                          {reportData.map((item, idx) => (
                             <tr key={idx} className="hover:bg-gray-50/30 transition-colors">
                                ...
                                <td className="p-4 md:p-6">
                                   <div className="font-black text-gray-900 mb-1 leading-tight text-xs md:text-sm">{item.topic || 'محاضرة أكاديمية معتمدة'}</div>
                                   <div className="text-[9px] md:text-[10px] text-gray-400 font-bold flex gap-3">
                                      <span className="px-2 py-0.5 rounded" style={{color: centerConfig.primary_color, backgroundColor: centerConfig.primary_color+'10'}}>{item.courseName}</span>
                          ...
                          {reportData.length === 0 && (
                            <tr><td colSpan="4" className="p-16 md:p-24 text-center text-gray-300 font-black italic tracking-widest text-[10px] md:text-xs uppercase underline underline-offset-8 decoration-dashed">لا توجد سجلات حالية للعرض.</td></tr>
                          )}
                       </tbody>
                    </table>
                  </div>
            ...
            <div className="section-container">
               <div className="flex items-center gap-4 md:gap-5 mb-8 md:mb-10 border-r-8 pr-4 md:pr-6 border-indigo-600">
                  <h3 className="text-xl md:text-2xl font-black text-gray-900 leading-none">نتائج الاختبارات والتقييمات</h3>
               </div>
            ...
            <div className="section-container mt-12 md:mt-20 p-8 md:p-12 bg-gray-50/50 rounded-3xl md:rounded-[4rem] border border-gray-100 flex flex-col md:flex-row justify-between items-center md:items-end gap-12 md:gap-0 print:flex print:mt-10">
                <div className="text-center space-y-3 md:space-y-4">
                    <p className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">توقيع ولي الأمر</p>
                    <div className="w-40 md:w-48 h-px bg-gray-300"></div>
                </div>
                <div className="flex flex-col items-center gap-4 relative">
                    <div className="absolute -top-10 md:-top-12 opacity-5 rotate-12" style={{color: centerConfig.primary_color}}>
                        <FaStamp size={64} className="md:w-20 md:h-20" />
                    </div>
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-dashed border-gray-200 flex items-center justify-center text-gray-200 font-black text-[9px] md:text-[10px] text-center p-3 uppercase tracking-tighter leading-tight">
                        ختم المركز<br/>الرسمي
                    </div>
                </div>
                <div className="text-center space-y-3 md:space-y-4">
                    <p className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">توقيع الإدارة</p>
                    <div className="w-40 md:w-48 h-px bg-gray-300"></div>
                </div>
            </div>
          ...
          <div className="bg-gray-950 p-10 md:p-16 text-white/90 print:bg-white print:text-gray-400 print:border-t-4 print:border-gray-50 print:p-14">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 md:gap-12 print:flex print:justify-between">
                <div className="space-y-4 text-center md:text-right w-full md:w-auto">
                   <p className="text-2xl md:text-3xl font-black leading-none tracking-tighter uppercase">{centerConfig.center_name}</p>
                   <p className="text-xs md:text-sm font-bold opacity-60 max-w-sm leading-loose border-r-0 md:border-r-2 pr-0 md:pr-6 mx-auto md:mx-0" style={{borderColor: centerConfig.primary_color}}>{centerConfig.report_footer}</p>
                </div>
                ...
                <div className="flex flex-col items-center md:items-end gap-5 w-full md:w-auto">
                   <div className="flex flex-col sm:flex-row md:flex-col gap-3 md:gap-2 mb-2 md:mb-4 w-full justify-center md:justify-end">
                      {centerConfig.center_phone && (
                        <div className="flex items-center gap-3 text-xs md:text-sm font-black opacity-80 justify-center md:justify-end">
                           <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><FaWhatsapp className="text-emerald-400"/></div>
                           <span className="tracking-wider">{centerConfig.center_phone}</span>
                        </div>
                      )}
                      {centerConfig.address && (
                        <div className="flex items-center gap-3 text-xs md:text-sm font-black opacity-60 justify-center md:justify-end">
                           <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><FaMapMarkerAlt className="text-gray-400"/></div>
                           <span className="text-[10px] md:text-xs text-center md:text-right">{centerConfig.address}</span>
                        </div>
                      )}
                   </div>
                   <div className="opacity-20 font-black text-[8px] md:text-[9px] uppercase tracking-[0.2em] md:tracking-[0.4em] flex flex-col gap-1 items-center md:items-end text-center md:text-right">
                      <span>Powered by Classora Intelligence Platform</span>
