...
export default function StudentDashboard() {
  ...
  // Show loading state if no centerId
  if (!centerId) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">جاري تحميل بيانات المركز...</p>
        </div>
      </div>
    );
  }
...
{/* 🔔 أزرار التحكم العلوية (تنبيهات + خروج) */}
      <div className="max-w-4xl mx-auto mb-4 flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <button 
            onClick={() => {
              ...
              toast.promise(handleLogout(), {
                ...
                error: 'حدث خطأ أثناء الخروج'
              });
            }}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-2xl text-[10px] font-black transition-all shadow-sm border bg-red-50 text-red-600 border-red-200 hover:bg-red-100"
          >
            <FaSignOutAlt />
            تسجيل خروج
          </button>
          ...
          <button 
            ...
            className={`w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-2xl text-[10px] font-black transition-all shadow-sm border ${isPushEnabled ? 'bg-green-50 text-green-600 border-green-200' : 'bg-white text-blue-600 border-blue-100 hover:bg-blue-50'}`}
          >
            {isPushEnabled ? <FaBell /> : <FaBellSlash />}
            {isPushEnabled ? "التنبيهات مفعلة" : "تفعيل التنبيهات"}
          </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-8 shadow-xl border border-gray-100 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl flex items-center justify-center shadow-lg overflow-hidden shrink-0">
                {centerSettings.logo_url ? (
                  ...
                  <span className="text-white text-xl md:text-2xl font-black">{centerSettings.logo}</span>
                )}
              </div>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 font-bold text-sm">إليك جدول حصصك وتنبيهاتك اللحظية</p>
          </div>
      ...
      {/* كروت الإحصائيات */}
      <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
        <div className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border border-blue-50 flex flex-col items-center justify-center transition-all hover:shadow-md">
            <FaWallet className="text-blue-500 mb-2 text-sm md:text-base" />
            <p className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-tighter">المحفظة</p>
            <p className="text-sm md:text-lg font-black text-blue-700">{stats.wallet} ج</p>
        </div>
        <div className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border border-red-50 flex flex-col items-center justify-center transition-all hover:shadow-md">
            <FaFileInvoiceDollar className="text-red-500 mb-2 text-sm md:text-base" />
            <p className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-tighter">المديونية</p>
            <p className="text-sm md:text-lg font-black text-red-700">{stats.debt} ج</p>
        </div>
        <div className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border border-green-50 flex flex-col items-center justify-center transition-all hover:shadow-md relative overflow-hidden">
            <p className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase mb-1 tracking-tighter">الالتزام</p>
            <p className="text-sm md:text-lg font-black text-green-600">{stats.rate}%</p>
        ...
        <div 
            ...
            className={`group p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border flex flex-col items-center justify-center transition-all cursor-pointer relative overflow-hidden ${earnedBadges.length > 0 ? 'bg-indigo-50/50 border-indigo-200 hover:shadow-2xl hover:shadow-indigo-100/50 hover:-translate-y-2' : 'bg-white border-indigo-50'}`}
        >
            <span className={`text-base md:text-xl mb-1 ${earnedBadges.length > 0 ? 'animate-bounce' : ''}`}>🏆</span>
            <p className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-tighter mb-0.5">الأوسمة</p>
            <p className="text-sm md:text-xl font-black text-indigo-700 leading-none">{earnedBadges.length}</p>
            ...
            {earnedBadges.length > 0 ? (
                ...
                <span className="text-[6px] md:text-[8px] font-bold text-slate-300 mt-2 italic">اجتهد لتنال أول وسام 🚀</span>
            )}
        </div>
      </div>

      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {/* الحصة القادمة - Hero Card مع وميض ذكي */}
        <div className={`lg:col-span-2 bg-gradient-to-br ${nextLesson?.exam_id ? 'from-rose-600 to-orange-700 border-rose-900/50' : 'from-blue-600 to-indigo-800 border-indigo-900/50'} rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-center min-h-[220px] md:min-h-[250px] border-b-8 transition-all duration-1000 ${timeLeft.isNow ? 'ring-8 ring-green-500/30 animate-pulse' : ''}`}>
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-4">
                <span className="bg-white/20 backdrop-blur-md px-3 md:px-4 py-1 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                   {nextLesson?.exam_id ? <FaFileAlt className={!timeLeft.isNow ? "animate-bounce" : ""} /> : <FaHourglassHalf className={!timeLeft.isNow ? "animate-spin-slow" : ""} />}
                   {timeLeft.isNow ? "بدأت الآن" : (nextLesson?.exam_id ? "موعد الاختبار القادم" : "العد التنازلي للحصة")}
                </span>
            </div>
            <h2 className="text-2xl md:text-4xl font-black mb-3 break-words">
                {nextLesson ? (nextLesson.exam_id ? `📝 ${nextLesson.exams?.title}` : nextLesson.groups?.courses?.name) : 'استمتع بوقتك'}
            </h2>
            {nextLesson && (
              <div className="flex flex-wrap gap-2 md:gap-4 mb-4 text-[10px] md:text-sm">
            ...
            <div className="text-[40px] leading-none font-black font-mono tracking-tighter mb-6">
                {timeLeft.isNow ? "🟢" : (
                  <div className="flex flex-row-reverse gap-2 justify-center">
                    {timeLeft.days > 0 && (
                      <div className="flex flex-col items-center">
                        <span className="text-3xl">{timeLeft.days.toString().padStart(2, '0')}</span>
                        <span className="text-[10px] text-blue-200">يوم</span>
                      </div>
                    )}
                    <div className="flex flex-col items-center">
                      <span>{timeLeft.hours.toString().padStart(2, '0')}</span>
                      <span className="text-[10px] text-blue-200">ساعة</span>
                    </div>
                    <span className="text-blue-200 self-center">:</span>
                    <div className="flex flex-col items-center">
                      <span>{timeLeft.minutes.toString().padStart(2, '0')}</span>
                      <span className="text-[10px] text-blue-200">دقيقة</span>
                    </div>
                    <span className="text-blue-200 self-center">:</span>
                    <div className="flex flex-col items-center">
                      <span>{timeLeft.seconds.toString().padStart(2, '0')}</span>
                      <span className="text-[10px] text-blue-200">ثانية</span>
                    </div>
                  </div>
                )}
            </div>
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-2xl backdrop-blur-sm border border-white/5">
                ...
                <span className="font-bold text-xs">
                  {nextLesson?.start_time ? (() => {
                    let [h, m] = nextLesson.start_time.split(':');
              ...
              <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-2xl backdrop-blur-sm border border-white/5">
                <FaMapMarkerAlt className="text-blue-200" /> <span className="font-bold text-xs">قاعة: {nextLesson?.rooms?.name || '---'}</span>
              </div>
            </div>
          </div>
          <FaChalkboardTeacher className="absolute -bottom-10 -left-10 text-white/5 text-[220px] rotate-12" />
        </div>

        {/* Dynamic QR Code */}
        <div className="bg-white rounded-[2.5rem] p-6 shadow-xl border border-gray-100 flex flex-col items-center justify-center text-center relative overflow-hidden transition-all hover:shadow-2xl">
            <FaSync size={10} className="absolute top-4 left-4 text-gray-200 animate-spin-slow" />
            <h3 className="text-gray-800 font-black text-xs mb-6 flex items-center gap-2"><FaQrcode className="text-blue-600" /> هويتك الرقمية</h3>
            <div className={`p-4 rounded-[2rem] border-2 border-dashed transition-all duration-500 shadow-inner mb-4 ${timeLeft.isNow ? 'border-green-400 bg-green-50 scale-110' : 'border-gray-200 bg-gray-50'}`}>
                {qrToken ? <QRCodeSVG value={qrToken} size={130} level="H" /> : <div className="w-[130px] h-[130px] bg-gray-200 animate-pulse rounded-2xl"></div>}
            </div>
