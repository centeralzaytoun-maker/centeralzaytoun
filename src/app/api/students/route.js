...
export async function GET(req) {
  try {
    ...
    const groupCourseId = searchParams.get('groupCourseId');
  ...
  query = query.ilike('grade', `%${grade}%`);
    ...
    // 🏷️ Group filter
    if (groupId && groupCourseId) {
      const groupFilterObj = { [groupCourseId]: groupId };
      query = query.contains('group_ids', JSON.stringify(groupFilterObj));
    }
