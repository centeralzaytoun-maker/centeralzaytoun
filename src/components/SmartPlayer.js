  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 z-10 ${showPlayer ? 'block' : 'hidden'}`}>
        ...
        {/* �️ CLASSORA CUSTOM CONTROL BAR */}
        <div
          className="absolute inset-x-0 bottom-0 z-50 px-5 pb-4 pt-10 bg-gradient-to-t from-black/95 via-black/50 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300"
          onContextMenu={(e) => e.preventDefault()}
        ...
        >
          ...
          <div className="flex items-center justify-between text-white">
            ...
            <div className="flex items-center gap-3">
              ...
              {playerReady && duration > 0 && (
                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black transition-all duration-500 ${
                  ...
                  <span>{watchedPct >= 85 ? '✅' : watchedPct >= 50 ? '⚠️' : '👁️'}</span>
                  <span>شاهدت {watchedPct}%</span>
                  {watchedPct < 85 && <span className="text-white/20">/ 85%</span>}
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 bg-blue-600/10 hover:bg-blue-600 text-blue-400 hover:text-white px-4 py-2 rounded-xl border border-blue-500/20 transition-all font-black text-[10px]"
            >
              {isFullscreen ? 'تصغير' : 'كامل الشاشة ⛶'}
            </button>
          </div>
        </div>
      </div>
